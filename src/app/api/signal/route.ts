...
import { validateRequest, SignalRequestSchema, validationErrorResponse } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!text) {
      logger.warn({ source }, 'Signal POST: missing text');
      return NextResponse.json({ success: false, error: "Text is required" }, { status: 400 });
    }
    
    logger.info({ source, textLength: text.length }, 'Signal received');
    ...
    if (!parsedSignal) {
      logger.warn({ text: text.substring(0, 200) }, 'Signal parse failed');
      return NextResponse.json({
        success: false,
    ...
    // Additional Zod validation for structured signal data
    const signalValidation = validateRequest(SignalRequestSchema, {
      symbol: parsedSignal.symbol,
      direction: parsedSignal.direction,
      action: parsedSignal.action,
      marketType: parsedSignal.marketType,
      entryPrices: parsedSignal.entryPrices,
      entryZone: parsedSignal.entryZone,
    ...
    if (!signalValidation.success) {
      logger.warn({ error: signalValidation.details, symbol: parsedSignal.symbol }, 'Signal schema validation failed');
      return NextResponse.json(
        validationErrorResponse(signalValidation.details),
        { status: 400 }
      );
    }
    ...
    if (!saveResult.success) {
      logger.error({ error: saveResult.error, symbol: parsedSignal.symbol }, 'Signal save failed');
      return NextResponse.json({
        success: false,
    ...
    logger.info({ signalId: saveResult.signalId, symbol: parsedSignal.symbol }, 'Signal saved successfully');
    ...
    logger.error(error, 'POST signal error', { duration });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!signalId) {
      logger.warn({ body }, 'Signal PUT: missing signalId');
      return NextResponse.json({ success: false, error: "signalId is required" }, { status: 400 });
    }
    
    logger.info({ signalId, takeProfits, stopLoss, status }, 'Signal update request');
    ...
    logger.error(error, 'PUT signal error');
