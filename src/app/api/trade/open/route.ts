...
import { validateRequest, TradeRequestSchema, validationErrorResponse } from "@/lib/validation";
import { 
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validation.success) {
      logger.warn({ error: validation.details }, 'Trade request validation failed');
    ...
    // Additional leverage validation for specific exchanges
    const targetExchangeId = validation.data.exchangeId || "binance";
    ...
    logger.info(
      { symbol, direction, amount, leverage, exchangeId: targetExchangeId },
      'Trade request received'
    );

    // Determine trading mode
    const actualTradingMode: TradingMode = tradingMode || (isDemo ? "DEMO" : "LIVE");
    ...
    logger.error(error, 'Trade open error', { duration });
    return NextResponse.json(
      { 
