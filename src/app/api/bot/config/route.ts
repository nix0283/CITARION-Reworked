...
import { validateRequest, BotConfigRequestSchema, validationErrorResponse } from "@/lib/validation";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validation = validateRequest(BotConfigRequestSchema, body);
    if (!validation.success) {
      logger.warn({ error: validation.details }, 'Bot config validation failed');
      return NextResponse.json(
    ...
    logger.info({ configId: id, name, exchangeId }, 'Bot config save request');
    ...
    logger.error(error, 'Save bot config error', { duration });
    return NextResponse.json(
      { 
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      logger.warn({ params: Object.fromEntries(searchParams) }, 'Bot config delete: missing ID');
      return NextResponse.json(
    ...
    logger.error(error, 'Delete bot config error');
    return NextResponse.json(
