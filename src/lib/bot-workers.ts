...
export async function processGridBot(botId: string): Promise<{
  ...
  try {
    ...
    for (let i = 0; i < gridLevels.length; i++) {
      ...
      if (i < currentLevel) {
        // Below current price - should have BUY order filled, now place SELL
        if (existingOrder && existingOrder.status === "FILLED" && existingOrder.side === "BUY") {
          ...
          if (!sellOrder) {
            ...
            const profit = (sellPrice - levelPrice) * existingOrder.filled;
            totalProfit += profit;
          ...
          // No order exists, create initial BUY order
          await db.gridOrder.create({
            data: {
              gridBotId: bot.id,
              gridLevel: i + 1,
              ...
              side: "BUY",
              status: "PENDING",
              amount: bot.perGridAmount || (bot.totalInvestment / bot.gridCount),
            },
          });
          
          actions.push(`Level ${i + 1}: Placed initial BUY at ${levelPrice.toFixed(2)}`);
        }
      } else if (i > currentLevel) {
        // Above current price - should have SELL order filled, now place BUY
        if (existingOrder && existingOrder.status === "FILLED" && existingOrder.side === "SELL") {
          // Check if buy order already exists
          const buyOrder = bot.gridOrders.find(o => o.gridLevel === i + 1 && o.side === "BUY");
          if (!buyOrder) {
            const buyPrice = gridLevels[Math.max(i - 1, 0)];
            const profit = (levelPrice - buyPrice) * existingOrder.filled;
            totalProfit += profit;
          ...
          // No order exists, create initial SELL order
          await db.gridOrder.create({
            data: {
              gridBotId: bot.id,
              gridLevel: i + 1,
          ...
          actions.push(`Level ${i + 1}: Placed initial SELL at ${levelPrice.toFixed(2)}`);
        }
      }
    }
